...
<?php
    ...
    if (Yii::$app->user->can(Constant::PERMISSION_MAIN_TAG.CoreConstant::PERMISSION_ACTION_UPDATE) === true)  {
        echo Html::beginTag('a', ['href' => Url::to(['tag/update', 'id' => $model->id]), 'class' => 'text-indigo-500 hover:text-indigo-700 transition icon-link', 'title' => 'Editer']);
        ?>
    ...
    if (Yii::$app->user->can(Constant::PERMISSION_MAIN_TAG.CoreConstant::PERMISSION_ACTION_DELETE) === true)  {
        echo Html::beginTag('a', ['href' => Url::to(['api/tag/delete', 'id' => $model->id]), 'class' => 'icon-link text-red-500 hover:text-red-700 transition user-button-delete', 'title' => 'Supprimer']);
        ?>
        <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
