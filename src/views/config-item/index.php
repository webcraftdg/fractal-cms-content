...
<div class="fc-list-header">
    <div>
        <h2 class="fc-list-header-title">Liste des configurations des éléments</h2>
    </div>
    <div>
        <?php
        if (Yii::$app->user->can(Constant::PERMISSION_MAIN_CONFIG_ITEM.CoreConstant::PERMISSION_ACTION_CREATE) === true):

            echo Html::beginTag('a', ['href' => Url::to(['config-item/create']), 'class' => 'fc-btn fc-btn-success']);
            ?>
        ...
        <?php
            ...
            foreach ($models as $model) {
             ...
             echo Html::beginTag('div', ['class' => implode(' ', $classes), 'fractal-cms-core-list-line' => $model->id]);
                 echo Html::tag('div', '#'.$model->id.' '.ucfirst($model->name), ['class' => 'flex-1 font-medium']);
                 echo Html::beginTag('div', ['class' => 'flex items-center gap-3 sm:w-auto']);
                     if (Yii::$app->user->can(Constant::PERMISSION_MAIN_CONFIG_ITEM.CoreConstant::PERMISSION_ACTION_UPDATE) === true)  {
                         echo Html::beginTag('a', ['href' => Url::to(['config-item/update', 'id' => $model->id]), 'class' => 'text-indigo-500 hover:text-indigo-700 transition icon-link', 'title' => 'Editer']);
                         ?>
                         <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        if (Yii::$app->user->can(Constant::PERMISSION_MAIN_CONFIG_ITEM.CoreConstant::PERMISSION_ACTION_DELETE) === true)  {
                            echo Html::beginTag('a', ['href' => Url::to(['api/config-item/delete', 'id' => $model->id]), 'class' => 'icon-link text-red-500 hover:text-red-700 transition user-button-delete', 'title' => 'Supprimer']);
                            ?>
                            <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
